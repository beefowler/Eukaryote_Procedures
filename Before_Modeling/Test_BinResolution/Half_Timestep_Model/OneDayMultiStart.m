...
%Outputs: 
%modelresults is 1 x 23 vector = [day xmin fmin mu mu1 mu2 p1 p2 exitflag length(modelfits)]
...
% mu mu1 mu2 p1 p2 are results of growth_rate function for these paramters
...
ub=[1 15 1000 1 1 15 1000 1 0.5 100 100 30 30 1e4]; %upper bounds 
...
problem = createOptimProblem('fmincon','x0',x0,'objective',@(theta) negloglike_calc(Einterp,dayCONC,theta,volbins,hr1,hr2),'Aineq',A,'bineq',B,'options',opts);
[~,~,~,~,soln] = run(ms,problem,tpoints);
    ...
    for j=1:length(soln)
        ...
        if length(g)==14 %only one start_point led to that solution
            ...
            temp(c,16)=growth_rate(Einterp,volbins,dayCONC,temp(c,1:13),hr1,hr2);
            ...
            temp(c:c+num-1,16)=repmat(growth_rate(Einterp,volbins,dayCONC,temp(c,1:13),hr1,hr2),num,1);
    ...
    [mu, mu1, mu2 pp] =growth_rate(Einterp,volbins,dayCONC,xmin(1:13),hr1,hr2);

    modelresults=[day xmin fmin mu mu1 mu2 pp(1,end) pp(2,end) exitflag length(modelfits)]; %this is how the optimization results are stored in model output files 

    
% %Step 5: Simulate according to results of optimization 
% 
[simCONC, simPROPS] = simdata(Einterp,dayCONC,xmin,volbins,hr1,hr2);
