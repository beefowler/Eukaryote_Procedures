...
dt=(1/12); %Half normal timestep 1/12 corresponds to 5 minutes!
...
for t=1:(1/dt)      
     ...
     A(stasis_ind)=(1-delta)*(1-y(t+(1/dt)*hr)); % stasis, the (1/dt)*hr part in the indexing is because eahc hour is broken up inot 10min segments for the irradiance spline
     A(m,m)=(1-delta(m));
                 
     A(growth_ind)=y(t+(1/dt)*hr)*(1-delta(1,1:m-1)); %growth on sub-diagonal
