%% This script is for forming the input data for our model from the FCB data on picoeuakaryotes or syn 
...
%         end
   ...
   %
    ...
    %
    %Is a full day present?
    %if dielhr(1) <= 5 && dielhr(end) == 24 && isempty(find(diff(dielhr) > 2, 1)),  %full day ==> proceed, otherwise just go to next day
    if dielhr(1) == 0 && dielhr(end) == 24 && isempty(find(diff(dielhr) > 2,1))  %full day ==> proceed, otherwise just go to next day
        ...
        if dielstarthr == 0,
            ...
            eval(['save ' modelpath 'day' num2str(day) 'data day volbins Edata Vhists N_dist cellsperml dielstarthr'])  %save data for rerunning batch later
        end;
    else
    ...
    if filenum == numfiles && day >= daylist(end), %case of last file and last day (untested as of 2/16/02)
        ...
        day = daylist(dayind);
        w=find_yearday(day);
        dielstarthr = dawn(find(dawn(:,1) == day),2) + CONST;
        if isnan(dielstarthr), dielstarthr = 0; end;
        if isempty(dielstarthr), dielstarthr = 0; end;
