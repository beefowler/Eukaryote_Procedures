...
function [mu mu1 mu2 pp]=growth_rate_phours_13params_plateau(Einterp,volbins,CONC,theta,hr1,hr2)
...
%mu_mat = [NaN; NaN; NaN;]; %hourly growth rates
pp = [sum(Nt1(:,1))./ sum(Nt1(:,1) + Nt2(:,1)); sum(Nt2(:,1))./ sum(Nt1(:, 1)+Nt2(:,1))]; 
%proportions 
...
for t=1:q %go through all the hours of interest
    ...
    pp = [pp [sum(Nt1(:,t+1))./ sum(Nt1(:,t+1) + Nt2(:,t+1)); sum(Nt2(:,t+1))./ sum(Nt1(:,t+1)+Nt2(:,t+1))]]; 
